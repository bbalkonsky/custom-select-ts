<template>
  <div tabindex="0" class="cselect" :class="{'is-active': isSelectActive}"
       @keyup.up="onArrowUp()" @keyup.down="onArrowDown()">
...
<script lang="ts">
  ...
  export default class CSelect extends Vue {
    ...
    onOptionClick(selected: Options): void {
      this.currentSelectIdx = this.optionList
          .findIndex(item => item.label === selected.label && item.value === selected.value);
    ...
    // альтернативой было делать разные компоненты в шаблоне для разных типов входных параметров
